...
传统「读文档学安全」往往抽象、难记忆；纯工具演示又缺少情境代入。《星辉剑传》把 **剧情推进**、**可玩的对话关卡** 和 **可观测的模型行为** 绑在同一条体验链路里，让玩家在扮演勇者与星辉剑对话的过程中，理解攻击面与防御思路。

| 常见痛点 | 本项目做法 |
|----------|------------|
| 概念抽象、难共情 | 章节化 RPG 剧情 + 会说话的星辉剑人设 |
| 攻击演示与剧情脱节 | 每关 `seal` 系统提示词 + 关卡目标关键词/判定 |
| 前后端分散部署麻烦 | FastAPI 同端口挂载 `Front-end development` |
| 多模型/多关配置复杂 | `chapter1/config.py` 与环境变量统一路由模型 |

当前面向 **课堂演示、安全科普、实验性 Playground**。完整生产级 LLM 应用安全体系还包括红队流程、内容审核、审计日志与合规策略等；本仓库聚焦 **可复现的关卡式体验**，在有限角色集合（玩家、游戏 API、云端模型）之间构建适合教学的主链路。
## 🎮 核心内容
