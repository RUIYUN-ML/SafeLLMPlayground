...
from chapter1.config import BIG_MODEL, resolve_siliconflow_model_id
from chapter2.http_budget import HttpCallBudget, chapter2_request_budget_seconds
...
# 与 level1 一致走 SiliconFlow；可用环境变量单独指定本关模型
SWORD_MODEL = os.getenv(
    "SILICONFLOW_CHAPTER2_LEVEL2_MODEL",
    os.getenv("DASHSCOPE_CHAPTER2_LEVEL2_MODEL", BIG_MODEL),
)
BLACKSMITH_GENERATOR_MODEL = os.getenv(
    "SILICONFLOW_CHAPTER2_LEVEL2_BLACKSMITH_MODEL",
    os.getenv("DASHSCOPE_CHAPTER2_LEVEL2_BLACKSMITH_MODEL", BIG_MODEL),
)
...
async def _call_sword_model(
    ...
    *,
    ...
    try:
        mid = resolve_siliconflow_model_id(name)
...
async def run_level2_game(user_training: str) -> dict:
    budget = HttpCallBudget(chapter2_request_budget_seconds())
    ...
    t_r1 = budget.next_call_timeout(3)
    if t_r1 <= 0:
        return {
            ...
            "sword_reply": "Error when querying API: 本关多轮模型调用时间预算已耗尽，请稍后重试。",
            "sword_reply_round1": None,
            ...
            "result": {
                ...
                "round1_leaked": False,
                "round2_leaked": None,
                "reason": "服务器侧多轮调用时间预算不足，请稍后重试。",
                "outcome": "error",
                "flavor_text": "本关需在约半分钟内完成多轮模型调用；当前无法发起第一轮请求，请稍后重试。",
                "api_error": True,
            },
        }

    sword_r1 = await _call_sword_model(
        system_prompt,
        user_message_r1,
        http_timeout=t_r1,
        max_tokens=384,
    ...
    if api_error_r1:
        return {
            ...
            "result": {
                ...
                "reason": display_r1,
                "outcome": "error",
                "flavor_text": "无法调用硅基流动大模型，本局未作泄密判定。请检查 siliconflow_api 中密钥/网络，或重试。",
                "api_error": True,
            },
        }
    ...
    t_gen = budget.next_call_timeout(2)
    if t_gen <= 0:
        return {
            "secret": LEVEL2_SECRET,
            "user_training": user_training,
            ...
            "result": {
                ...
                "round1_leaked": False,
                "round2_leaked": None,
                "reason": "第一轮已完成，但剩余时间不足以安全调用第二轮台词生成，请缩短输入或稍后重试。",
                "outcome": "error",
                "flavor_text": "多轮模型调用接近浏览器超时上限；请稍后重试或简化训练词。",
                "api_error": True,
            },
        }
    ...
    blacksmith_line = await _call_sword_model(
        bs_system,
        BLACKSMITH_GENERATOR_USER_MESSAGE,
        model=BLACKSMITH_GENERATOR_MODEL,
        http_timeout=t_gen,
        max_tokens=192,
    )
    ...
    t_r2 = budget.next_call_timeout(1)
    if t_r2 <= 0:
        return {
            "secret": LEVEL2_SECRET,
            "user_training": user_training,
            ...
            "result": {
                ...
                "round1_leaked": False,
                "round2_leaked": None,
                "reason": "第二轮台词已生成，但剩余时间不足以调用星辉剑作答，请稍后重试。",
                "outcome": "error",
                "flavor_text": "多轮模型调用接近浏览器超时上限；请稍后重试。",
                "api_error": True,
            },
        }

    sword_r2 = await _call_sword_model(
        system_prompt,
        display_bl,
        http_timeout=t_r2,
        max_tokens=384,
