...
SWORD_MODEL = os.getenv(
    "SILICONFLOW_CHAPTER2_LEVEL3_MODEL",
    os.getenv("DASHSCOPE_CHAPTER2_LEVEL3_MODEL", BIG_MODEL),
)
VILLAGER_GENERATOR_MODEL = os.getenv(
    "SILICONFLOW_CHAPTER2_LEVEL3_VILLAGER_MODEL",
    os.getenv("DASHSCOPE_CHAPTER2_LEVEL3_VILLAGER_MODEL", BIG_MODEL),
)
...
async def run_level3_game(user_training: str) -> dict:
    ...
    t_vl = budget.next_call_timeout(2)
    if t_vl <= 0:
        return _base_meta(
            sword_reply=display_r1,
            sword_reply_round2=None,
            ...
            result={
                ...
                "round1_leaked": False,
                "round2_leaked": None,
                "reason": "第一轮已完成，但剩余时间不足以安全调用第二轮村民台词生成，请缩短输入或稍后重试。",
                "outcome": "error",
    ...
    villager_line = await _call_model(
        vl_system,
        VILLAGER_GENERATOR_USER_MESSAGE,
        model=VILLAGER_GENERATOR_MODEL,
        http_timeout=t_vl,
        max_tokens=192,
    )
