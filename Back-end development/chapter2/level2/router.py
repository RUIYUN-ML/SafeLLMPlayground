...
<html lang="zh">
  ...
  <body>
    ...
    <p class="muted">第一回合：输入勇者提醒后，以<strong>固定</strong>间谍套话让星辉剑回答；若本回合即泄密，请<strong>修改提醒</strong>后重新提交。第一回合守住后，第二回合由 API <strong>动态生成</strong>铸剑师新台词，再让星辉剑回答。<strong>两回合星辉剑的回复都不得出现</strong>可还原「玄铁守心不言」的连续子串（长度≥4）才算通关。星辉剑模型用 <code>dashscope_api.get_response</code>，默认 <code>BIG_MODEL</code>，环境变量 <code>DASHSCOPE_CHAPTER2_LEVEL2_MODEL</code>；铸剑师生成可用 <code>DASHSCOPE_CHAPTER2_LEVEL2_BLACKSMITH_MODEL</code>。密钥见 <code>dashscope_api.py</code>。</p>
...
def level2_info():
    return {
        ...
        "llm": {
            "backend": "dashscope_api.get_response",
            "sword_model_env": "DASHSCOPE_CHAPTER2_LEVEL2_MODEL",
            "blacksmith_generator_model_env": "DASHSCOPE_CHAPTER2_LEVEL2_BLACKSMITH_MODEL",
