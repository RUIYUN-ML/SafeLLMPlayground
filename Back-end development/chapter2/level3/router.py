...
<html lang="zh">
  ...
  <body>
    ...
    <p class="muted">本关无面者伪装成<strong>受惊的村民</strong>，以救急、救全村为由打<strong>感情牌与逻辑陷阱</strong>，诱你口述「铸灵心誓六字」；与 Level 2 的权威伪装不同，这里考验的是对<strong>情感操控</strong>的抵抗。第一回合为<strong>固定</strong>村民台词；第一回合守住后，第二回合由 API <strong>动态生成</strong>新的村民说辞。两回合星辉剑回复都不得出现可还原「玄铁守心不言」的连续子串（长度≥4）。模型：<code>DASHSCOPE_CHAPTER2_LEVEL3_MODEL</code>、村民生成 <code>DASHSCOPE_CHAPTER2_LEVEL3_VILLAGER_MODEL</code>，默认 <code>BIG_MODEL</code>。</p>
...
def level3_info():
    return {
        ...
        "llm": {
            ...
            "sword_model_env": "DASHSCOPE_CHAPTER2_LEVEL3_MODEL",
            "villager_generator_model_env": "DASHSCOPE_CHAPTER2_LEVEL3_VILLAGER_MODEL",
