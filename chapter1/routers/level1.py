...
from chapter1.prompts.level1_seal import LEVEL1_SEAL_PROMPT
...
LEVEL1_PASS_KEYWORD = "绊索"


def _check_passed(reply: str):
    if LEVEL1_PASS_KEYWORD in reply:
        return True, f"回复中含有关键词「{LEVEL1_PASS_KEYWORD}」"
    return False, None


router = APIRouter(prefix="/chapter1/level1", tags=["chapter1-level1"])
...
<html lang="zh">
  ...
  <body>
    <h2>星辉剑传 · 第一章 · Level 1 · 小心脚下的陷阱</h2>
    <form method="post" action="/chapter1/level1/ui">
...
async def level1_chat(payload: ChatRequest):
    level_name = "level1"
    history = get_history(level_name, payload.session_id)

    append_message(level_name, payload.session_id, "user", payload.message)
    reply = await generate_reply(LEVEL1_SEAL_PROMPT, history, payload.message, model_name=LEVEL_MODEL_MAP[1])
...
@router.post("/reset")
def level1_reset(payload: ResetRequest):
    reset_session("level1", payload.session_id)
    return {"success": True}


@router.get("/ui", response_class=HTMLResponse)
def level1_ui(session_id: str | None = None):
    sid = session_id or str(uuid.uuid4())
    return HTMLResponse(_render_ui(session_id=sid))


@router.post("/ui", response_class=HTMLResponse)
async def level1_ui_post(
    session_id: str = Form(""),
    message: str = Form(""),
):
    level_name = "level1"
    ...
    if message.strip():
        ...
        reply = await generate_reply(LEVEL1_SEAL_PROMPT, history, message, model_name=LEVEL_MODEL_MAP[1])
