...
from chapter1.prompts.level3_seal import LEVEL3_SEAL_PROMPT
...
LEVEL3_PASS_KEYWORD = "黎明之烛"

_CENSOR_WORD = "无面"


def _censor_reply(reply: str) -> str:
    """将 NPC 回复中的关键答案词（整词或任意单字）替换为遮蔽词，模拟血咒封印效果。"""
    result = reply.replace(LEVEL3_PASS_KEYWORD, _CENSOR_WORD)
    for char in LEVEL3_PASS_KEYWORD:
        result = result.replace(char, _CENSOR_WORD)
    return result


def _check_passed(reply: str, user_message: str = "") -> tuple:
    """通关判定改为检查玩家输入：玩家自己说出「火把」才算破解封印。"""
    if LEVEL3_PASS_KEYWORD in user_message:
        return True, f"勇者说出了正确答案「{LEVEL3_PASS_KEYWORD}」"
    return False, None


router = APIRouter(prefix="/chapter1/level3", tags=["chapter1-level3"])
...
<html lang="zh">
  ...
  <body>
    <h2>星辉剑传 · 第一章 · Level 3 · 狼群封印</h2>
    <form method="post" action="/chapter1/level3/ui">
...
async def level3_chat(payload: ChatRequest):
    level_name = "level3"
    history = get_history(level_name, payload.session_id)

    append_message(level_name, payload.session_id, "user", payload.message)
    raw_reply = await generate_reply(LEVEL3_SEAL_PROMPT, history, payload.message, model_name=LEVEL_MODEL_MAP[3])
    reply = _censor_reply(raw_reply)
    append_message(level_name, payload.session_id, "assistant", reply)

    passed, reason = _check_passed(reply, payload.message)
    return ChatResponse(
        reply=reply,
        passed=passed,
...
@router.post("/reset")
def level3_reset(payload: ResetRequest):
    reset_session("level3", payload.session_id)
    return {"success": True}


@router.get("/ui", response_class=HTMLResponse)
def level3_ui(session_id: str | None = None):
    sid = session_id or str(uuid.uuid4())
    return HTMLResponse(_render_ui(session_id=sid))


@router.post("/ui", response_class=HTMLResponse)
async def level3_ui_post(
    session_id: str = Form(""),
    message: str = Form(""),
):
    level_name = "level3"
    ...
    if message.strip():
        ...
        raw_reply = await generate_reply(LEVEL3_SEAL_PROMPT, history, message, model_name=LEVEL_MODEL_MAP[3])
        reply = _censor_reply(raw_reply)
        append_message(level_name, sid, "assistant", reply)
        passed, reason = _check_passed(reply, message)
